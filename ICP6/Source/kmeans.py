...
sns.FacetGrid(dataset, hue="TENURE", size=5).map(plt.scatter, "BALANCE", "PURCHASES").add_legend()
# do same
sns.FacetGrid(dataset, hue="TENURE", size=5).map(plt.scatter, "CREDIT_LIMIT", "PAYMENTS").add_legend()
plt.show()
